window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#endregion
		//#region helpers
		/** Upper bound on automatic history pages pulled while the tab is open
		*  (50 messages per page server-side): a backstop against a host that
		*  keeps advertising `hasMore` without delivering new nodes. */
		const MAX_AUTO_PAGES = 200;
		/** Find the DOM row bound to one chat node key (ui-chat/ui-conversation
		*  stamps every message row with data-chat-anchor-key in both versions). */
		...
		/**
		* Legacy view switch: simulate a click on the Chat tab button. Pre-0.1.5
		* profiles expose no openView prop, so the tab bar is the only handle.
		* @returns whether a tab button was clicked.
		*/
		...
		/**
		* Build the directory from the ordered chat node keys + keyed node store.
		* Tolerant of an absent source so a version skew renders the empty state
		* instead of crashing the tab.
		* @param order - ordered chat node keys.
		* @param nodes - keyed chat node store (`get(key)`).
		* @returns directory rows in session order.
		*/
		function buildItems(order, nodes) {
			const out = [];
			if (!Array.isArray(order) || nodes === void 0 || nodes === null || typeof nodes.get !== "function") return out;
		...
		//#region TocBody
		/**
		* Shared directory body: rows, auto-paging, click-to-jump and flash. Both
		* data-source variants render through this component so the interaction
		* logic lives in exactly one place.
		* @param props - rows, paging state, the loadOlder action, and the
		*   version-appropriate view switcher.
		* @returns the directory element tree.
		*/
		function TocBody({ items, hasMore, loadingOlder, loadOlderPage, switchToChat, t }) {
			const flashTimerRef = react.useRef(null);
			react.useEffect(() => () => {
				if (flashTimerRef.current !== null) clearTimeout(flashTimerRef.current);
			}, []);
			/** Auto-page: while the tab is mounted and more history exists, keep
			*  pulling older pages until `hasMore` clears. The session's own
			*  loadOlder guard (open && hasMore && !loadingOlder) plus the
			*  loadingOlder dependency bound this to one page at a time, and the
			*  page budget backstops a host that keeps reporting hasMore while
			*  returning nothing. */
			const pagesRef = react.useRef(0);
			react.useEffect(() => {
				if (!hasMore || loadingOlder) return;
				if (pagesRef.current >= MAX_AUTO_PAGES) return;
				pagesRef.current += 1;
				loadOlderPage();
		...
		//#endregion
		//#region TocPanel variants
		/**
		* Current DSH (>= 0.1.5): chat nodes moved out of the session snapshot into
		* the `useChat` standard hook (provided by dsh-client-ui-chat through
		* uiSession.provide); cross-view switching is the official `openView` prop.
		*/
		function TocPanelCurrent({ useChat, useSession, loadOlderPage, openView, t }) {
			const order = useChat((s) => s.order);
			const nodes = useChat((s) => s.nodes);
			const hasMore = useSession((s) => s.hasMore);
			const loadingOlder = useSession((s) => s.loadingOlder);
			const items = react.useMemo(() => buildItems(order, nodes), [order, nodes]);
			const switchToChat = react.useCallback(() => {
				if (typeof openView === "function") openView("chat");
				else switchToChatTab();
			}, [openView]);
			return react.createElement(TocBody, {
				items,
				hasMore,
				loadingOlder,
				loadOlderPage,
		...
		/**
		* Legacy DSH (<= 0.1.0-rc.x): chat nodes rode the session snapshot at
		* `s.chat.*`; no openView prop exists, so the Chat tab is clicked directly.
		*/
		function TocPanelLegacy({ useSession, loadOlderPage, t }) {
			...
			return react.createElement(TocBody, {
				...
				loadingOlder,
				loadOlderPage,
				switchToChat: switchToChatTab,
				t
			});
		}
		/**
		* Version dispatcher. The presence of the `useChat` standard hook decides
		* which variant mounts — each variant calls its own hooks unconditionally,
		* keeping the rules-of-hooks contract intact.
		*/
		function TocPanel(props) {
			return props.useChat === void 0
				? react.createElement(TocPanelLegacy, props)
				: react.createElement(TocPanelCurrent, props);
		}
		//#endregion
		//#region apply
			...
			ctx.effect(() => ctx.slots.inject("conversation.view", () => ctx.slots.register({
				...
				inject: (sessionId) => {
					...
					return {
						...
						loadOlderPage: () => session.loadOlder()
					};
				}
